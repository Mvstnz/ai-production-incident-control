...
[Open the application](https://ai-production-incident-control-dash.vercel.app) and select **Explore the workspace**.
...
Supplier emails are stored in PostgreSQL and never delivered externally. The hosted instance uses deterministic extraction; Gemini calls are disabled. Database writes affect only the synthetic ERP.
...
Implemented, locally tested and target-tested boundaries are recorded separately in the [acceptance matrix](../../acceptance/acceptance-matrix.json). Live model accuracy, real supplier delivery, load testing and disaster recovery remain NOT_RUN.
