...
const allowedCredentials = new Set(['APIC Operations Service','APIC ERP Read','APIC Webhook Intake','APIC Demo Form']);
...
function inspect(expected, actual) {
  ...
  const credentialBindings = expected.nodes.filter(n=>n.credentials).map(n=>({
    node:n.name, required:n.credentials, returned:actualByName.get(n.name)?.credentials || {}, status:'BLOCKED_MISSING_CREDENTIAL'
  }));
...
module.exports = {build,inspect,hash,canonical,edges,ORDER};
