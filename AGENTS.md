...
**Paketversion 1.1 — maßgeblich: `CODEX_BUILD_SPEC.md`, M0–M7, 36 Abnahmekriterien.**
...
## Korrigierte Übergabe und fehlende Verbindung

Alte `docs/BUILD_PLAN.md`-Vorgaben mit M0–M5 sind abgelöst. `RESUME_AFTER_HANDOFF_FIX.md` beschreibt die Fortsetzung ohne Verlust bestehender Arbeit. Keine alte und neue Geschäftslogik vermischen; vorhandene Sicherheitsregeln nicht durch diese Spezifikation aufheben.

Ein im Auftrag angekündigter n8n-Connector ist noch kein tatsächlich verfügbares Tool. Tool-Discovery und gegebenenfalls lokale MCP-Metadaten prüfen, ohne Secrets auszugeben. Kein Connectorname und keine Instanz-URL dürfen geraten werden. Fehlender Zielzugriff: `BLOCKED_TARGET_CONNECTION` dokumentieren, M6/AC31 nicht als bestanden melden und M1–M5 sowie unabhängige Teile von M7 fortsetzen. Wenn auch lokale Runtime-Voraussetzungen fehlen, nur die davon betroffenen Tests als BLOCKED/NOT_RUN führen, nicht die gesamte Umsetzung stoppen. Bereits genehmigte API-/CLI-Alternativen nach Abschnitt 5.3 bleiben zulässig.
