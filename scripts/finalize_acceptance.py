...
matrix.update(spec_version='1.1',evaluated_at=datetime.now(timezone.utc).isoformat(),note='Current retained evidence for ordinary manufacturing examples. Local, connected and live-model checks are distinguished.')
rows={x['id']:x for x in matrix['criteria']}
for row in rows.values():
    row.update(status='NOT_RUN',evidence=[],verification_scope='DEMO_LOCAL')
    row.pop('note',None)
...
target=read('n8n/target/manifest.json')
hosted=read('evidence/workflow-runs/hosted-demo.json')
actions=read('evidence/workflow-runs/hosted-actions.json')
target_ok=target['status']==hosted['status']==actions['status']=='TARGET_TESTED' and all(w['active'] and w['published_matches_draft'] for w in target['workflows'])
set_result('AC31','PASS' if target_ok else 'NOT_RUN','evidence/workflow-runs/hosted-demo.json','evidence/workflow-runs/hosted-actions.json','n8n/target/manifest.json',note='Three connected scenario analyses and six successful approved sandbox actions; public read-only access and private-scope isolation checked.')
rows['AC31']['verification_scope']='CONNECTED_SANDBOX'
evaluation=read('evidence/evaluations/fixture-v2-report.json')
set_result('AC32','PASS' if evaluation['status']=='LOCAL_TESTED' else 'NOT_RUN','evidence/evaluations/fixture-v2-report.json',note='50 deterministic fixture cases; development/holdout denominators and versions reported. LIVE_EVAL_NOT_RUN; no model-performance claim.')
browser=read('evidence/test-results/browser.json') if (ROOT/'evidence/test-results/browser.json').exists() else {'checks':[]}
matches=[x for x in browser['checks'] if 'AC36' in x.get('criteria',[])]
set_result('AC36','PASS' if matches and all(x['status']=='PASS' for x in matches) else 'NOT_RUN','evidence/test-results/browser.json','evidence/workflow-runs/hosted-actions.json')
